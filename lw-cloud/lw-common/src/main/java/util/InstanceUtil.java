...
//    protected static log log = LogManager.getLogger(InstanceUtil.class);
    ...
    // Bean --> Map 1: 利用Introspector和PropertyDescriptor 将Bean --> Map
    public static Map<String, Object> transBaseModelList2Map(List<? extends BaseModel> list) {
        Map<String, Object> map = newHashMap();
        ...
        try {
            for (BaseModel obj : list) {
                map.put(String.valueOf(obj.getId()), obj);
            }
        } catch (Exception e) {
            log.error("transBaseModel2Map Error ", e);
        }
        return map;
    }
    ...
    private static Map<String, MethodAccess> methodMap = new ConcurrentHashMap<>();
     ...
     * @return
     */
    public static boolean isInstanceFull(Object bean) {
        if (bean == null) {
            return false;
        }
